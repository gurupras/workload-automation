...
class FPSInstrument(Instrument):
    ...
    def setup(self, context):
        ...
        self.fps_outfile = os.path.join(context.output_directory, 'fps.json')
        self.collector = FPSCollector(self.period_ms, self.fps_outfile, self.device, self.logger)
        self.device.execute('service call SurfaceFlinger 1001 i32 {}'.format(self.period_ms))
...
class FPSCollector(threading.Thread):
    ...
    def run(self):
        try:
            ...
            fd, fps_file = tempfile.mkstemp()
            self.logger.debug('fps file: {}'.format(fps_file))

            deviceid = self.device.adb_name
            fps_data = []
            cmd = 'adb -s {} logcat -v tracetime SurfaceFlinger:* *:S'.format(deviceid)
            try:
                ...
                while not self.stop_signal.is_set():
                    ...
                    for line in stdout:
                        ...
                        if logline is None:
                            ...
                            continue
                        try:
                            fps_json = json.loads(logline['payload'])
                            self.logger.debug(fps_json)
                            fps_data.append(fps_json)
                        except:
                            self.logger.error("Failed to parse json: '{}'".format(logline['payload']))
            finally:
                pass
        ...
        with open(self.outfile, 'wb') as wfh:
            wfh.write(json.dumps(fps_data, indent=2))
